...
def perceptual_style_tv_loss(masked, y_true, y_pred, mask):
    '''
    ...
    '''
    y_comp = mask * masked + (1 - mask) * y_pred
    ...
    for i in range(len(features_pred)):
        ...
        style_loss += loss_fn(gram_matrix(features_true[i]),
                              gram_matrix(features_pred[i]))
        style_loss += loss_fn(gram_matrix(features_true[i]),
                              gram_matrix(features_comp[i]))
...
def loss(masked, y_true, y_pred, mask):
    hole_loss, valid_loss = pixel_loss(y_true, y_pred, mask)
    perceptual_loss, style_loss, tv_loss = perceptual_style_tv_loss(
        masked, y_true, y_pred, mask)
    total_loss = lambda_valid * valid_loss + lambda_hole * hole_loss + \
        lambda_perceptual * perceptual_loss + \
        lambda_style * style_loss + lambda_tv * tv_loss
    return total_loss
