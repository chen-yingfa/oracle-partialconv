...
class UpConcat(nn.Module):
    ...
    def forward(self, x, x_rhs, mask, mask_rhs):
        x = self.up(x)
        x = self.concat(x, x_rhs)
        mask = self.up(mask)
        mask = self.concat(mask, mask_rhs)
        return x, mask


def up_concat(x, x_rhs, mask, mask_rhs):
    return UpConcat()(x, x_rhs, mask, mask_rhs)

class PConvNet(nn.Module):
    def __init__(self):
        super().__init__()
        # self.channels = [64, 128, 256, 512, 512, 512, 512, 512]
        self.channels = [16, 32, 64, 128, 128, 128, 128, 128]
        self.enc_kernels = [7, 5, 5, 3, 3, 3, 3, 3]
        self.dec_kernels = 3
        # 7*7对应padding=3, 5*5的两个是2, 3*3的是1
        self.enc_padding = [3, 2, 2, 1, 1, 1, 1, 1]
        self.PConv1 = PR(
            3, self.channels[0], self.enc_kernels[0], self.enc_padding[0])
        # 这个第一层独立出来，因为没有BN
    ...
    def forward(self, x, mask):
        # x: (B, 3, 128, 128)
        enc_x1, enc_mask1 = self.PConv1(x, mask)  # (B, 16, 64, 64)
        enc_x2, enc_mask2 = self.PConv2(enc_x1, enc_mask1)  # (B, 32, 32, 32)
        enc_x3, enc_mask3 = self.PConv3(enc_x2, enc_mask2)  # (B, 64, 16, 16)
        enc_x4, enc_mask4 = self.PConv4(enc_x3, enc_mask3)  # (B, 128, 8, 8)
        enc_x5, enc_mask5 = self.PConv5(enc_x4, enc_mask4)  # (B, 128, 4, 4)
        enc_x6, enc_mask6 = self.PConv6(enc_x5, enc_mask5)  # (B, 128, 2, 2)
        enc_x7, enc_mask7 = self.PConv7(enc_x6, enc_mask6)  # (B, 128, 1, 1)
        # enc_x8, enc_mask8 = self.PConv8(enc_x7, enc_mask7)  # (B, 512,2,2)
        ...
        dec_x7, dec_mask7 = up_concat(enc_x7, enc_x6, enc_mask7, enc_mask6)   # 512+512
        dec_x7, dec_mask7 = self.PConv12(dec_x7, dec_mask7)       # 512+512->512

        dec_x6, dec_mask6 = up_concat(dec_x7, enc_x5, dec_mask7, enc_mask5)   # 512+512
        dec_x6, dec_mask6 = self.PConv11(dec_x6, dec_mask6)       # 512+512->512

        dec_x5, dec_mask5 = up_concat(dec_x6, enc_x4, dec_mask6, enc_mask4)   # 512+512
        dec_x5, dec_mask5 = self.PConv12(dec_x5, dec_mask5)       # 512+512->512

        dec_x4, dec_mask4 = up_concat(dec_x5, enc_x3, dec_mask5, enc_mask3)   # 512+256
        dec_x4, dec_mask4 = self.PConv13(dec_x4, dec_mask4)       # 512+256->256

        dec_x3, dec_mask3 = up_concat(dec_x4, enc_x2, dec_mask4, enc_mask2)   # 256+128
        dec_x3, dec_mask3 = self.PConv14(dec_x3, dec_mask3)       # 256+128->128

        dec_x2, dec_mask2 = up_concat(dec_x3, enc_x1, dec_mask3, enc_mask1)   # 128+64
        dec_x2, dec_mask2 = self.PConv15(dec_x2, dec_mask2)       # 128+64->64

        dec_x1, dec_mask1 = up_concat(dec_x2, x, dec_mask2, mask)        # 64+3
